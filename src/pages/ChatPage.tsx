...
amplitude.init('367cde3fdffdda095fd5224e90020801', {"autocapture":{"attribution":true,"fileDownloads":true,"formInteractions":true,"pageViews":true,"sessions":true,"elementInteractions":true,"networkTracking":true,"webVitals":true,"frustrationInteractions":true}});
...
const ChatPage: React.FC = () => {
  ...
  // Ensure iframe refreshes when previewUrl changes (new preview or deploy)
  useEffect(() => {
    if (previewUrl) {
      // Bump cache-busting so iframe reloads even if browser caches aggressively
      setLastRefreshTime(Date.now());
    }
  }, [previewUrl]);

  // HEAD check for container status
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        ...
        setIsContainerOnline(res.status === 200);
      } catch {
        if (!isCancelled) setIsContainerOnline(false);
      }
    };
    checkStatus();
    return () => {
  ...
  }, [previewUrl, lastRefreshTime]);

  // Ensure iframe refreshes after a modification completes even if the URL stays identical
  useEffect(() => {
  ...
  const handleDeploy = useCallback(async () => {
    ...
    try {
      ...
      // Update preview URL to the deployed URL
      setPreviewUrl(deployedUrl);
    } catch (error) {
  ...
  const generateCacheBustingUrl = useCallback((baseUrl: string) => {
    ...
    url.searchParams.set("_t", `${timestamp}_${random}`);
  ...
  const forceIframeRefresh = useCallback(
    async (method: "key" | "src" | "location" | "all" = "all") => {
      ...
      try {
        ...
        if (method === "key" || method === "all") {
          // Method 1: Change React key to force remount
          setIframeKey((prev) => prev + 1);
          setLastRefreshTime(Date.now());
        }

        if (method === "src" || method === "all") {
          // Method 2: Update src with cache busting
          const cacheBustedUrl = generateCacheBustingUrl(previewUrl);
        ...
        if (method === "location" || method === "all") {
          // Method 3: Use contentWindow.location (if same origin)
          try {
            if (iframe && iframe.contentWindow) {
              const cacheBustedUrl = generateCacheBustingUrl(previewUrl);
              iframe.contentWindow.location.href = cacheBustedUrl;
            }
          } catch (e) {
            console.error("Cross-origin iframe, using src method only");
          }
        }

        // If using 'all' method, stagger the approaches
        if (method === "all") {
          setTimeout(() => {
            if (iframe) {
              // Force a second refresh with empty src first, then set new URL
              iframe.src = "";
              setTimeout(() => {
                const finalUrl = generateCacheBustingUrl(previewUrl);
                iframe.src = finalUrl;
    ...
    [previewUrl, generateCacheBustingUrl, setPreviewUrl]
  );

  // Generate cache-busted preview URL
  const cacheBustedPreviewUrl = React.useMemo(() => {
    if (!previewUrl) return null;
    return generateCacheBustingUrl(previewUrl);
  }, [previewUrl, lastRefreshTime, generateCacheBustingUrl]);
    ...
    <>
      ...
      >
        ...
        <div
          ...
          style={{ width: isChatHidden ? '0%' : `${leftWidthPct}%` }}
        >
                ...
                <div className="chat-buttons">
                  <button
                    onClick={() => {
                      ...
                      // Only stopWorkflow if streaming
                      else if (
                        isStreamingResponse ||
                        isStreamingGeneration ||
                        isStreamingModification
                    ...
                    disabled={
                      // Disable if:
                      // - No prompt when idle
                      // - Running but NOT streaming (user can't click)
                      // - Stopping (after user clicked, but process not yet stopped)
                      !(
                        prompt.trim() &&
                        !isLoading &&
                        !isWorkflowActive &&
                  ...
                  >
                    {/* Spinner visible ONLY if process is running but NOT streaming */}
        ...
        {!isChatHidden && (
          <div
            ...
            className={`relative z-40 flex-shrink-0 cursor-col-resize hover:bg-blue-200 transition-colors`}
            style={{ width: 6, cursor: "col-resize" }}
          >
            <div className="h-full w-[2px] bg-slate-300 mx-auto" />
          </div>
        )}
        ...
        <div
          ...
          style={{ width: isChatHidden ? '100%' : `${100 - leftWidthPct}%` }}
        >
          {/* Preview Header */}
          <div className="header-bar">
            <div className="flex items-center justify-between w-full h-10">
              ...
              <div className="flex items-center gap-2 h-8">
                ...
                {projectStatus === "ready" && (
                  <div className="flex items-center h-8">
                    <Dialog>
                      <DialogTitle className="hidden">Manage GitHub</DialogTitle>
                      <DialogTrigger
                      ...
                      <DialogContent className="bg-white w-[95vw] sm:max-w-3xl max-w-[95vw] max-h-[85vh] overflow-auto p-0">
                ...
                {previewUrl && (
                  <div className="flex items-center h-8">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center justify-center w-8 h-8 text-primary hover:text-primary-dark transition-colors rounded-md hover:bg-primary-subtle"
                      title="Open in new tab"
                    >
                      <ExternalLink className="w-5 h-5 text-primary hover:scale-105 transition-transform duration-200" />
                    </a>
                  </div>
                )}
                ...
                {/* Deploy Button */}
                <div className="relative flex items-center h-8" data-publish-menu>
                  <button
                  ...
                  {showPublishMenu && (
                    <>
                      {/* Fullscreen transparent overlay to catch outside clicks (including iframe) */}
                      <div className="publish-overlay" onMouseDown={() => setShowPublishMenu(false)} />
                      <div className="publish-dropdown z-dropdown fade-slide-in" onAnimationEnd={(e) => e.currentTarget.classList.remove('fade-slide-in')}>
                        ...
                        {shareAbleUrl && (
                          <div className="publish-preview">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <a className="publish-preview-link" href={shareAbleUrl} target="_blank" rel="noopener noreferrer">
                                {shareAbleUrl}
                              </a>
                            </div>
                          </div>
            ...
            <div className="w-full h-full panel-elevated overflow-hidden relative">
              ...
              {activeTab === "preview" ? (
                ...
                <>
                  ...
                  {previewUrl && isContainerOnline === true ? (
                    <iframe
                      ref={iframeRef}
                      key={iframeKey} // Force remount when key changes
                      src={cacheBustedPreviewUrl || previewUrl}
