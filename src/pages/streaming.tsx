...
export const StreamingCodeDisplay: React.FC<{
  ...
  const handleEditorError = useCallback((error: any, filename: string) => {
    console.error(`Monaco Editor Error for ${filename}:`, error);
...
export class StreamingCodeParser {
  ...
  static parseStreamingChunk(chunk: string): {
    ...
    const cacheKey = `${chunk.length}-${chunk
      .slice(0, 100)
      .replace(/\s/g, "")}`;
    ...
    try {
      ...
      console.error(
        "LLMCodeParser JSON parsing failed, trying fallback methods:",
        e
      );
    }
        ...
        for (const pattern of jsonPatterns) {
          ...
          for (const match of matches) {
            try {
              ...
              console.error("JSON parse attempt failed:", e);
            }
          }
          if (files.length > 0) break;
        }
      } catch (e) {
        console.error("JSON parsing failed, trying fallback methods:", e);
      }
    }
    ...
    if (files.length === 0) {
      try {
        ...
        console.error("Direct pattern parsing failed:", e);
      }
    }
    ...
    if (currentFile && uniqueFiles.length > 0) {
      const fileIndex = uniqueFiles.findIndex(
        (f) => f.filename === currentFile
      );
      if (fileIndex !== -1) {
        progress = Math.max(
          progress,
          ((fileIndex + 0.5) / uniqueFiles.length) * 100
        );
      }
    }
