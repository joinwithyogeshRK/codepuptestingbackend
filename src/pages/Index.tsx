  ...
  ({
    ...
    const getCleanProjectName = (name: string) => {
      ...
      try {
        ...
        // console.error("🔍 ProjectCard - Error parsing project name:", e);
        return name;
      }
    };
    ...
    const designChoices = useMemo(() => {
      try {
        ...
        console.error("Error parsing design choices:", error);
      }
      return null;
    }, [project.description]);
        ...
        <div className="project-thumbnail">
          {project.deploymentUrl ? (
            ...
            <div className="project-thumbnail-placeholder">
              <Code2 className="project-thumbnail-icon" />
            </div>
          )}
...
const Index = () => {
  ...
  const checkBackendCapabilities = useCallback(async () => {
    try {
      ...
      if (
        ...
        console.warn("⚠️ Session support not available, using limited mode");
        setHasSessionSupport(false);
        setBackendStatus("limited");
      }
    } catch (error) {
      console.error("❌ Could not check backend capabilities:", error);
      if (axios.isAxiosError(error)) {
        console.error("❌ Health check error status:", error.response?.status);
        console.error("❌ Health check error data:", error.response?.data);
      }
      setHasSessionSupport(false);
      setBackendStatus("limited");
    }
  ...
  const updateProjectName = useCallback(
    async (projectId: number, newProjectName: string) => {
      if (!dbUser) return;
      ...
      try {
        ...
        if (updateResponse.data.success) {
          // Update the local projects state to reflect the new name
  ...
  const startAnalyzeWorkflow = useCallback(
    async (projectId: number, userPrompt: string) => {
      ...
      try {
        ...
        if (analyzeResponse.data.success) {
          ...
          // Update project name if LLM returned a new one, or use businessName as fallback
          let newProjectName = processedDesignChoices?.projectName;

          if (!newProjectName && processedDesignChoices?.businessName) {
            // Convert businessName to lowercase with hyphens as fallback
            newProjectName = processedDesignChoices.businessName
              .toLowerCase()
              .replace(/\s+/g, "-");
          }
        ...
        console.error("Error starting analyze workflow:", error);
    ...
    [dbUser, selectedImages, updateProjectName, projects, selectedProjectType]
  );
  ...
  const handleFeedbackSubmit = useCallback(
    async (feedback: string) => {
      ...
      try {
        ...
        if (feedbackResponse.data.success) {
          ...
          if (newProjectName && newProjectName !== currentProject?.name) {
            await updateProjectName(currentProjectId, newProjectName);
          }
    ...
    [currentProjectId, dbUser, updateProjectName, projects]
  );
      ...
      try {
        ...
        try {
          ...
          if (axios.isAxiosError(userError)) {
            console.error(
              "❌ User creation error status:",
              userError.response?.status
            );
            console.error(
              "❌ User creation error data:",
          ...
          if (
            ...
            try {
              ...
              userResponse = await axios.get<DbUser>(
                `${BASE_URL}/api/users/${clerkUser.id}`,
                {
                  headers: {
                    Authorization: `Bearer ${token}`,
                  },
              ...
              if (axios.isAxiosError(fetchError)) {
                console.error(
                  "❌ User fetch error status:",
                  fetchError.response?.status
                );
                console.error(
                  "❌ User fetch error data:",
        ...
        try {
          ...
          if (!Array.isArray(fetchedProjects)) {
            ...
            if (
              ...
              console.warn(
                "❌ Received health response instead of projects data. Projects endpoint may not be available."
              );
            }
          ...
          if (axios.isAxiosError(projectError)) {
            console.error("❌ Response status:", projectError.response?.status);
            console.error("❌ Response data:", projectError.response?.data);
            console.error("❌ Request URL:", projectError.config?.url);
            console.error(
              "❌ Response headers:",
              projectError.response?.headers
            );
          }
          ...
          if (
            ...
            console.warn(
              "Projects endpoint not available, using empty projects list"
            );
            console.info(
              "💡 This is expected if you're running a simplified backend version"
            );
        ...
        console.error("❌ Critical error in user sync:", error);
  ...
  const handleLoadMoreProjects = useCallback(async () => {
    ...
    try {
      ...
      if (!Array.isArray(newProjectsRaw)) {
        console.error(
          "❌ Projects response is not an array for page",
          nextPage,
          newProjectsRaw
        );
