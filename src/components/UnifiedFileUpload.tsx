...
const UnifiedFileUploadSection = forwardRef<UnifiedFileUploadRef, UnifiedFileUploadSectionProps>(({
  selectedFiles,
  setSelectedFiles,
  isConfigValid,
  showToast,
  ...
  const extractImagesFromPdf = async (pdfFile: File) => {
    try {
      ...
      // Check page limit
      if (pdf.numPages > 5) {
        showToast(`PDF "${pdfFile.name}" has ${pdf.numPages} pages. Maximum 5 pages allowed.`, 'error');
        return [];
      }
      ...
      for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
        ...
        const imageFile = new File([blob], `${pdfFile.name}-page-${pageNum}.png`, {
          type: 'image/png',
      ...
      showToast(`Successfully extracted ${extractedImages.length} images from PDF`, 'success');
      ...
      if (errorMessage.includes('Invalid PDF')) {
        ...
        showToast('PDF.js worker not properly configured. Please refresh and try again.', 'error');
      } else {
  ...
  const handleFileSelect = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        const otherFiles = newFiles.filter((file: File) => 
        ...
        if (pdfFiles.length > 0) {
          showToast(`Processing ${pdfFiles.length} PDF file(s)...`, 'success');
          
          for (const pdfFile of pdfFiles) {
            // Check file size (3.75MB limit)
            if (pdfFile.size > 3.75 * 1024 * 1024) {
              showToast(`PDF file "${pdfFile.name}" is too large. Maximum size is 3.75MB`, 'error');
              continue;
            }
        ...
        for (const file of allImageFiles) {
          ...
          if (file.size > 3.75 * 1024 * 1024) {
            showToast(`Image "${file.name}" is too large. Maximum size is 3.75MB`, 'error');
            continue;
          }
        ...
        if (validImageFiles.length > 0) {
          ...
          setSelectedFiles(validImageFiles.slice(0, 1));
  ...
  const createImageFromText = async (text: string, filename: string): Promise<File> => {
    const canvas = document.createElement('canvas');
