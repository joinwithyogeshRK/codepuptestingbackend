  ...
  const handleFileSelect = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (supportedFiles !== newFiles.length) {
        showToast('Only image files (PNG, JPG, GIF) and PDF files are supported', 'error');
      }

      let allImageFiles = [...imageFiles];

      // Process PDF files if any - CONVERT TO IMAGES
      ...
      for (const file of allImageFiles) {
        ...
        if ((file as File).size > 3.75 * 1024 * 1024) {
          showToast(`Image "${(file as File).name}" is too large. Maximum size is 3.75MB`, 'error');
          continue;
        }
      ...
      <div className="relative">
        ...
        <label
          ...
          className={`image-upload-area ${
            isProcessingPdf ? 'image-upload-area-processing' : 'image-upload-area-enabled'
          }`}
        >
            ...
            return (
              <>
                ...
                {Object.entries(groups).map(([pdfName, images]) => {
                  ...
                  const mockPdfFile = new File([], pdfName, { type: 'application/pdf' });
                ...
                {standaloneImages.map((image: File, index: number) => (
                  <div key={`standalone-${index}`} className="image-preview-item">
                    <img
