...
const Credit: React.FC<CreditProps> = ({ value, className }) => {
  ...
  const [isLoading, setIsLoading] = React.useState<boolean>(value === undefined);
  ...
  return (
    <div
      className={
        `flex items-center gap-2 px-3 h-8 rounded-md bg-green-50 text-strong  transition-colors ${
          className || ""
        }`
      }
