...
import { Github, RefreshCw, Upload, ExternalLink, GitBranch, X, CheckCircle, AlertCircle } from "lucide-react";
...
export default function GitHubModel({ projectId, clerkId }: GitHubModelProps) {
  ...
  const [isConnectingProject, setIsConnectingProject] = useState<boolean>(false);
  ...
  const ENDPOINTS = {
    ...
    projectById: (projectId: number | string) => `${BASE_URL}/api/projects/${projectId}`,
    userDetails: (clerkId: string) => `${BASE_URL}/api/users/clerk/${clerkId}`,
  };

  // Toast notification functions
  const showToast = (message: string, type: 'success' | 'error', duration: number = 4000) => {
  ...
  const fetchProjectGithubUrl = async () => {
    ...
    try {
      ...
      console.error("Failed to fetch project GitHub URL:", err);
  ...
  const fetchUserDetails = async () => {
    ...
    try {
      ...
      const data: { githubAccessToken?: string; githubAvatar?: string; githubUsername?: string } = await res.json();
  ...
  const handleLogin = async () => {
    if (!GITHUB_CLIENT_ID || !clerkId) {
      showToast("Configuration error - missing GitHub Client ID or Clerk ID", 'error');
      return;
    }
    const state = { clerkId };
    const userData = JSON.stringify(state);
    const githubAuthUrl = `https://github.com/login/oauth/authorize?client_id=${GITHUB_CLIENT_ID}&scope=read:user repo workflow&state=${encodeURIComponent(userData)}`;
  ...
  const handleProjectSubmit = async (isUpdate = false) => {
    ...
    if (!zipUrl.trim() || !repoName.trim() || !clerkId) {
      showToast("Missing required project data. Please try refreshing the page.", 'error');
      return;
    }
    ...
    try {
      const requestBody = { zipUrl, reponame: repoName.trim(), clerkId, projectId };
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData || `${isUpdate ? "Update" : "Upload"} failed`);
      }
      ...
      showToast(`Project ${isUpdate ? "updated" : "connected"} successfully!`, 'success');
    } catch (err) {
      const message = err instanceof Error ? err.message : "Unknown error occurred";
      showToast(`Failed to ${isUpdate ? "update" : "connect"} project: ${message}`, 'error');
    } finally {
            ...
            <div className="w-full mx-auto">
              ...
              <div className="flex items-start justify-between mb-8">
                <div>
                  ...
                  <p className="text-gray-600 mt-1">Sync your project with GitHub for collaboration.</p>
                </div>
              </div>
              ...
              <section className="bg-white border border-gray-200 rounded-lg p-6 mb-6">
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                  <div>
                    ...
                    <p className="text-gray-600 mt-1 text-sm">Connect your GitHub account to enable project migration.</p>
                  </div>

                  {githubConnected ? (
                    <div className="flex items-center gap-3">
                      {githubAvatar && (
                        <img src={githubAvatar} alt="GitHub Avatar" className="w-8 h-8 rounded-full border" />
                      )}
                      <span className="text-sm text-gray-800">{githubUsername}</span>
              ...
              {!githubConnected && (
                <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                  <p className="text-yellow-800 text-sm">
                    Please connect your GitHub account first to enable project migration and repository creation.
                  </p>
                </div>
              )}
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    <div>
                      <h3 className="text-lg font-semibold flex items-center gap-2">
                        Project
                        <span className="bg-green-100 text-green-700 text-xs font-medium px-2 py-1 rounded-full">Connected</span>
                      </h3>
                      <p className="text-gray-600 text-sm mt-1">Your project is synced with GitHub.</p>
                    </div>
                    ...
                    <div className="flex gap-3">
                      ...
                      >
                        <RefreshCw className={`w-4 h-4 ${isUpdatingProject ? 'animate-spin' : ''}`} />
                ...
                <section className="bg-white border border-gray-200 rounded-lg p-6 mb-6">
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    ...
                    <button
                      ...
                      title={!githubConnected ? "Connect GitHub account first" : ""}
                    >
                      <Upload className={`w-4 h-4 ${isConnectingProject ? 'animate-pulse' : ''}`} />
                      {isConnectingProject ? "Connecting..." : "Connect Project"}
                    </button>
                  </div>
                </section>
              )}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {repoUrl && projectConnected && (
                  <div className="bg-white border border-gray-200 rounded-lg p-6">
                    <h3 className="text-lg font-semibold mb-2">Clone Repository</h3>
                    <p className="text-gray-600 text-sm mb-4">Copy this URL to clone your repository locally.</p>
